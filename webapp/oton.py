...
def _show_month(tm):
    ...
    a = [lunar_month(month), lunar_day(day)]
    return a
...
# 显示现在的日期
def this_month():
    a=show_month(datetime.now().year, datetime.now().month, datetime.now().day)
    return a
...
实现方法
1.从数据库取出所有的数据进行遍历
2.遍历的时候对数据进行判断，如果为新历则直接比较，如果相等则加入队列
3.如果为农历则转换后在比较，如果相等则加入队列
4.使用函数对队列进行处理
'''